...
import {
  LayoutDashboard, CloudSun, Sprout, TrendingUp, MessageSquare,
  Settings, Leaf, Moon, Sun, X, Menu,
} from "lucide-react";
...
function applyFont(font: string) {
  ...
  if (font && font !== "inter") {
    document.documentElement.classList.add(`font-theme-${font}`);
  }
}
...
const AppContent = () => {
  ...
  useEffect(() => {
    const font = localStorage.getItem("font_pref") || "inter";
    applyFont(font);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#f6f6f8] dark:bg-[#101013] text-stone-900 dark:text-[#e8e8ea] transition-colors duration-300">

      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex flex-col fixed inset-y-0 left-0 w-60 bg-white dark:bg-[#16161a] border-r border-stone-200/80 dark:border-stone-800/50 z-40">
        <div className="px-6 py-5 flex items-center gap-2.5 border-b border-stone-100 dark:border-stone-800/50">
        ...
        <div className="px-3 py-4 border-t border-stone-100 dark:border-stone-800/50 space-y-0.5">
      ...
      {/* Mobile Header */}
      <header className="lg:hidden sticky top-0 z-50 flex items-center justify-between px-4 h-14 bg-white/95 dark:bg-[#16161a]/95 backdrop-blur-xl border-b border-stone-200/80 dark:border-stone-800/50">
        ...
        {isMobileMenuOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="lg:hidden fixed inset-0 top-14 bg-black/20 z-40" onClick={() => setMobileMenuOpen(false)} />
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
              className="lg:hidden fixed top-14 left-0 right-0 z-50 bg-white dark:bg-[#16161a] border-b border-stone-200 dark:border-stone-800/50 px-3 py-2 space-y-0.5">
              {navItems.map(({ to, icon: Icon, label }) => (
                <NavLink key={to} to={to} end={to === "/"}
                  className={({ isActive }) =>
      ...
      <main className="lg:pl-60 min-h-screen">
        <AnimatePresence mode="wait">
          <motion.div key={location.pathname}
            ...
            transition={{ duration: 0.2, ease: "easeOut" }} className="min-h-screen">
            <Routes>
              ...
              <Route path="/settings" element={<SettingsPage onFontChange={applyFont} />} />
              <Route path="*" element={<Dashboard />} />
            </Routes>
          </motion.div>
        </AnimatePresence>
      ...
      {/* Mobile Bottom Nav */}
      <nav className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-white/95 dark:bg-[#16161a]/95 backdrop-blur-xl border-t border-stone-200/80 dark:border-stone-800/50 flex items-center justify-around px-2 h-16">
        {navItems.map(({ to, icon: Icon, label }) => (
          <NavLink key={to} to={to} end={to === "/"}
            className={({ isActive }) =>
